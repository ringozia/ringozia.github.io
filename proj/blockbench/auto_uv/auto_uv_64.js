(function() {
    ...
    BBPlugin.register('auto_uv_64', {
        title: '自动UV64',
        author: 'Ringozia',
        description: '适用于尺寸64*64贴图的自动UV，基岩版逐面UV专用',
        icon: 'border_inner',
        version: '3.0.0',
        variant: 'both',
        onload() {
            autoUV64Action = new Action('auto_uv_64_btn', {
                name: '自动UV(64*64贴图适用)',
                description: '64×适用*为该块所有面更新UV',
                ...
                click: function () {
                    ...
                    Cube.selected.forEach(cube => {
                        ...
                        ['north', 'south', 'east', 'west', 'up', 'down'].forEach(faceKey => {
                            ...
                            // === 核心功能：尺寸 * 0.25 ===
                            let targetW = dim.w * 0.25;
                            let targetH = dim.h * 0.25;
            ...
            // 添加到菜单栏 (Filter 菜单)
            MenuBar.addAction(autoUV64Action, 'filter');
            
            // 添加到 UV 编辑器工具栏
            if (typeof Toolbars !== 'undefined' && Toolbars.uv_dialog) {
                Toolbars.uv_dialog.add(autoUV64Action);
            }
        },
        onunload() {
            autoUV64Action.delete();
        }
    });
